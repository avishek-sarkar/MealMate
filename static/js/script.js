/**
 * MealMate - Frontend JavaScript
 * Professional food community platform
 */
...
function initializeModal() {
    ...
    if (closeBtn) {
        closeBtn.addEventListener('click', () => hideModal('loginModal'));
    }
...
function initializePostInteractions() {
    ...
    actionButtons.forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            if (icon && (icon.classList.contains('fa-thumbs-up') || icon.classList.contains('fa-heart'))) {
                toggleLikeButton(this, icon);
            }
        });
    });
}
...
function toggleLikeButton(button, icon) {
    ...
    if (icon.classList.contains('fas')) {
        ...
        if (countSpan) {
            let count = Math.max(0, parseInt(countSpan.textContent.trim()) - 1);
